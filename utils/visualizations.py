...
class VisualizationManager:
    ...
    def __init__(self):
        ...
        self.cmap = ListedColormap(['white', 'saddlebrown', 'black',
                                   'green', 'royalblue', 'red'])
        self.fig = None
        self.ax = None
        self.texts = {}
    ...
    def add_epsilon_slider(self, initial_eps, vacuum):
        ...
        slider = widgets.Slider(ax_eps, 'ε (Greedy)', 1.0, 5.0, valinit=initial_eps, valstep=0.1)
    ...
    def add_speed_slider(self):
        ...
        slider = widgets.Slider(ax_speed, 'Delay (s)', 0.01, 1.0, valinit=0.2, valstep=0.01)
    ...
    def update_display(self, env, vacuum, step):
        ...
        self.ax.set_title(f"Step {step} – {vacuum.strategy.upper()}  ε={vacuum.epsilon}",
                         fontsize=14, fontweight='bold')
        
        if 'battery' in self.texts:
            self.texts['battery'].set_text(str(vacuum.battery))
        if 'stats' in self.texts:
             self.texts['stats'].set_text(f"Cleaned: {vacuum.cleaned} | Visited: {len(vacuum.visited)}")
    ...
    def create_comparison_charts(self, results):
        ...
        axs[0,0].set_title('Steps'); axs[0,0].set_ylabel('count')
        ...
        for patch, col in zip(bp2['boxes'], colours):
            patch.set_facecolor(col); patch.set_alpha(0.7)
        axs[0,1].set_title('Efficiency'); axs[0,1].set_ylabel('dirt/step')
        ...
        for patch, col in zip(bp3['boxes'], colours):
            patch.set_facecolor(col); patch.set_alpha(0.7)
        axs[1,0].set_title('Coverage %'); axs[1,0].set_ylabel('percent')

        succ = []
        for s in strategies:
            total = len(results[s])
            if total == 0: succ.append(0)
            else:
        ...
        axs[1,1].set_title('Success Rate'); axs[1,1].set_ylabel('rate'); axs[1,1].set_ylim(0,1.1)
        for bar, v in zip(bars, succ):
            axs[1,1].text(bar.get_x()+bar.get_width()/2, v+0.02,
    ...
    def _draw_grid(self, env, vacuum, title):
        ...
        self.ax.legend(handles=legend_patches, loc='upper right', bbox_to_anchor=(1.15, 1))
        
        bat = self.ax.text(0.02, 0.98, str(vacuum.battery), transform=self.ax.transAxes,
                          va='top', bbox=dict(boxstyle='round', facecolor='lightblue'))
        stats = self.ax.text(0.02, 0.02, f"Cleaned: {vacuum.cleaned} | Visited: {len(vacuum.visited)}",
    ...
    def _build_display_array(self, env, vacuum):
        ...
        for y in range(env.size):
            for x in range(env.size):
                val = env.grid[y][x]
                if val == -1:          arr[y, x] = 2   # obstacle
                elif val == 1:         arr[y, x] = 1   # dirt
                elif val == 2:         arr[y, x] = 3   # charger
                elif val == 0:         arr[y, x] = 0   # clean
